...
class ServoMovementWorker(QObject):
    ...
    def __init__(self, arm_interface, positions, camera_manager, pose_estimator, save_dir, primary_camera_index=2, secondary_camera_index=4):
        super().__init__()
...
class ServosControlWidget(QWidget):
    ...
    def applyPositions(self):
        ...
        self.thread = QThread()
        self.worker = ServoMovementWorker(self.arm_interface, positions, self.camera_manager, self.pose_estimator, save_dir, primary_camera_index=2, secondary_camera_index=4)
